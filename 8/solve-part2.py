    ...
    for j in range(len(lines[0])):
        if lines[i][j] != '.':
            ...
            for combination in all_combinations:
                ...
                y_dist = (y1 - y2)
                x_dist = (x1 - x2)
                x_dist_list = [x_dist * i for i in range(len(lines))]
                y_dist_list = [y_dist * i for i in range(len(lines))]
                for x_dist_i, y_dist_i in zip(x_dist_list, y_dist_list):
                    for x,y,dir in [(x1,y1,1),(x2,y2,-1)]:
                        anti_x, anti_y = x+x_dist_i*dir, y+y_dist_i*dir
